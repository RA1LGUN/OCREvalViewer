# Pack public/doc_exports/ + public/pdfs/ into a zip that can be dragged into the web app.
# Usage:
...
$manifestPath = Join-Path $root "public\doc_exports\manifest.json"
if (-not (Test-Path $manifestPath)) {
    throw "public\doc_exports\manifest.json not found. Put exported data there first."
}
...
foreach ($entry in $manifest) {
    ...
    $srcJson = Join-Path $root "public\doc_exports\json\$jsonName"
    ...
    $pdfHits = Get-ChildItem -Recurse -Filter $entry.doc_name (Join-Path $root "public\pdfs") -ErrorAction SilentlyContinue
    if ($pdfHits) {
